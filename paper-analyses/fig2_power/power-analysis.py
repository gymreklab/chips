...
def main():
    ...
    for numreads in read_list:
        ...
        macs = 'macs2 callpeak --tempdir %s -t %s.flagged.bam -c %s.wce.flagged.bam --name %s.%s --outdir %s'%(os.path.dirname(output_prefix), output_prefix, output_prefix, os.path.basename(output_prefix), numreads, os.path.dirname(output_prefix))
        commands.append(macs)
        macs_broad = 'macs2 callpeak --tempdir %s --broad -t %s.flagged.bam -c %s.wce.flagged.bam --name %s.%s.broad --outdir %s'%(os.path.dirname(output_prefix), output_prefix, output_prefix, os.path.basename(output_prefix), numreads, os.path.dirname(output_prefix))
