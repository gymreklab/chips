...
with pysam.AlignmentFile(bamfile, "rb") as samfile:
    ...
    # frag_count_dict:
    #   key/frag location -> #identical frags
    frag_count_dict = {}
    for rn in ref_names:
        for frag in samfile.fetch(rn, 0, samfile.get_reference_length(rn)):
            if (frag.is_paired) and\
                ...
                frag_key = (frag_ref, frag_start, frag_mate_start)
...
# count_dict:
#   key/#identical frags: #occurrence
count_dict = {}
...
# if count_dict is geometric distributed:
ttl_count = 0
n_frags = 0
for count in count_dict:
    ttl_count += count*count_dict[count]
    n_frags += count_dict[count]
mean_val = ttl_count/float(n_frags)
p = 1.0/mean_val

print (count_dict)
print ("geometric distribution parameter p: %f" %(p))
